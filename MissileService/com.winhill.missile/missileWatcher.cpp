...
missileWatcher::missileWatcher(AllJoynBusAttachment^ busAttachment) :
...
missileWatcher::~missileWatcher()
{
    UnregisterFromBus();
}

void missileWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.winhill.missile" };
...
void missileWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.winhill.missile"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.winhill.missile"),
            port);
        Added(this, args);
    }
...
void missileWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<missileWatcher>
        };
...
void missileWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_missileIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &missileWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void missileWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void missileWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
